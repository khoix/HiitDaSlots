...
import LandingSubmenuReel, {
  type LandingSubmenuItem,
} from '@/components/LandingSubmenuReel';
import {
...
export default function LandingScreen({
  ...
  const landingSubmenuItems = useMemo((): LandingSubmenuItem[] => {
    const out: LandingSubmenuItem[] = [];
    if (onOpenCatalog) {
      out.push({
        key: 'catalog',
        label: 'Catalog',
  ...
  return (
    ...
    >
      ...
      {ctaReady && landingSubmenuItems.length > 0 ? (
        <div className="relative z-10 mt-6 w-full max-w-sm mx-auto">
          <LandingSubmenuReel items={landingSubmenuItems} />
        </div>
      ) : null}
